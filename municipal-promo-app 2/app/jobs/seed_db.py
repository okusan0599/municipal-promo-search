...
def _keep_municipality(row: dict) -> bool:
    ...
    if not city or not row.get("url") or not row.get("lgcode"):
        return False
    # localgovjp also includes designated-city administrative wards as "札幌市 中央区".
    # v6 manages the designated city as one contracting authority; Tokyo's 23 special wards
    # have no parent city prefix and are therefore retained.
...
def seed_rows(repo: ProjectRepository, municipalities: Iterable[dict], prefectures: Iterable[dict]) -> int:
    count = 0
    for row in prefectures:
        name = str(row.get("pref") or "").strip()
        if not name or not row.get("url") or not row.get("lgcode"):
            continue
        repo.upsert_municipality({"code": str(row["lgcode"]), "prefecture": name, "name": name, "kind": "prefecture",
                                  "official_url": row["url"], "area": AREA_BY_PREF.get(name), "active": True})
        count += 1
    for row in municipalities:
        if not _keep_municipality(row):
            continue
        pref = str(row.get("pref") or "").strip(); city = str(row.get("city") or "").strip()
        repo.upsert_municipality({"code": str(row["lgcode"]), "prefecture": pref, "name": city, "kind": _kind(city, pref),
...
def load_remote_rows() -> tuple[list[dict], list[dict]]:
    headers = {"User-Agent": "MunicipalPromotionSearch/6.0 seed-job"}
    m = requests.get(MUNICIPALITY_DATA_URL, timeout=60, headers=headers); m.raise_for_status()
    p = requests.get(PREF_DATA_URL, timeout=60, headers=headers); p.raise_for_status()
    return m.json(), p.json()


def seed_known_sources(repo: ProjectRepository) -> int:
    ...
    for row in rows:
        try:
            repo.upsert_source(row); saved += 1
        except ValueError:
            continue
    return saved
